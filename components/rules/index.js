  ...
  return (
    <div>
      <div
        ...
        } w-full h-full bg-black/40 top-0`}
      >
        <div className="fixed left-1/2 transform -translate-x-1/2 -translate-y-1/2 top-1/2 z-20">
          <motion.div
            className=" bg-white w-screen h-screen rounded-md sm:w-96 sm:h-96"
            layout
            animate={{ x: 0, y: 0, r: 0 }}
            transition={{ type: "just", stiffness: 100 }}
          >
            <div className="flex flex-col justify-center items-center h-full sm:h-auto">
              <div className="flex justify-center w-full p-6 sm:justify-between">
                <span className="relative text-3xl font-bold text-dark-text sm:text-2xl bottom-32 sm:bottom-auto ">
                  RULES
                </span>
                <button
                  className="absolute bottom-12 left-1/2 text-2xl font-medium tracking-widest font-sans text-gray-400 sm:relative sm:bottom-auto sm:left-auto sm:text-xl"
      ...
      <div className="absolute bottom-0 m-12 right-0">
        <button
          className="flex flex-row w-32 justify-center border rounded-md text-xl uppercase text-white p-2 tracking-wide"
          onClick={(e) => rulesButton(e)}
        >
          Rules
        </button>
      </div>
